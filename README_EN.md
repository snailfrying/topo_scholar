...
Official origin, meaning, and history records have been collected for all 31 province-level administrative regions, 333 collectable prefecture-level administrative regions, 2,793 county-level administrative regions via the national database, 70 manually verified supplemental and special-area records, and the Nangao Village example. Manual supplemental records currently include 49 high-confidence and 21 medium-confidence entries, with no `reference_only` records remaining; the next quality pass should replace media-derived records with original official/gazetteer sources where possible. See `data/metadata/quality_report.json` and `docs/quality_assessment.md` for the latest validation report.
...
## Local Frontend Browser

TopoScholar includes a dependency-free local browser UI for searching by keyword, category, province, source, confidence, and collection status. It also provides a province-level bubble map for quick coverage inspection.

```powershell
python frontend\server.py
```

Open `http://127.0.0.1:8766` after startup. The UI reads local SQLite/CSV files only; the map uses approximate province center points and is not an administrative-boundary map.
...
- Local government websites, civil-affairs pages, local gazetteers, and toponym gazetteers: verified supplemental sources for records not covered by the national database.
...
- `place_knowledge` now contains official origin records for all 31 province-level administrative regions, 333 collectable prefecture-level administrative regions, 2,793 county-level administrative regions via the national database, 70 manually verified supplemental and special-area records, 8,373 administrative-subdistrict pilot records, and the Nangao Village example. The current county/special-area queue has no pending rows and no `needs_review` rows; the administrative-subdistrict pilot queue has 739 `needs_review` rows for slower manual verification and no pending rows.
