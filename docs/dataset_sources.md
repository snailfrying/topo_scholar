...
| P2 | 地方政府/民政局/地方志/地名志 | 分散网页/PDF/图书 | 补充地名由来和历史沿革 | 已用于补齐 70 条国家地名信息库未命中的县级和特殊/功能区记录；需要证据抽取和版权控制 |

## 首批落地数据

第一批先拉取 `modood/Administrative-divisions-of-China` 作为五级行政区划基础底表，并保留原始文件到 `data/raw/`。

后续再按需接入国家地名信息库详情，并用地方政府/民政局/地方志资料补充未命中项，逐步补齐 `place_knowledge`。

## 许可与再分发边界

- 项目代码和项目自有文档采用 MIT License。
- 第三方数据和由第三方来源派生的知识记录不自动改用 MIT，仍遵守原始数据源的许可证、引用要求和使用边界。
- `place_knowledge.csv` 只应保存结构化摘要、短证据摘录、来源 URL、置信度和来源类型；不要提交原始网页全文、接口响应 JSON 或站点缓存。
- 坐标、边界、Shapefile、GeoJSON、地图瓦片等空间数据暂不进入普通 Git；如需发布，应单独进行许可证、体积和测绘/地理信息合规审查。
