...
![Knowledge](https://img.shields.io/badge/origin_records-11%2C602-b36b2c)
...
- 行政街道试点：已生成 9,112 条独立下一阶段队列，先快速采集 8,373 条官方来源记录，739 条进入 `needs_review` 慢慢复核。
...
项目内置一个零前端依赖的本地数据浏览界面，可按关键词、类别、省份、来源和队列状态筛选，并提供省级覆盖气泡地图。

```powershell
python frontend\\server.py
```

启动后打开 `http://127.0.0.1:8766`。前端只读取本地 SQLite/CSV，不会上传数据；地图使用省级近似中心点展示覆盖规模，不代表精确行政边界。
